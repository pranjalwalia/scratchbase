...
TEST_CASE("Measure basic performance","[setKeyValue,getKeyValue]") {
  ...
  //   [What]  I need to know the performance characteristics of each scratchbase storage class
  ...
  SECTION("Store and Retrieve 100 000 keys - In-memory key-value store") {
    ...
    std::string dbname("myemptydb");
    std::unique_ptr<scratchbase::KeyValueStore> memoryStore = std::make_unique<scratchbaseext::MemoryKeyValueStore>();
    std::unique_ptr<scratchbase::IDatabase> db(scratchbase::ScratchBase::createEmptyDB(dbname, memoryStore));
  ...
  SECTION("Store and Retrieve 100 000 keys - File based key-value store") {
    ...
    std::string dbname("myemptydb");
    std::string fullpath = ".scratchbase/" + dbname;
    std::unique_ptr<scratchbase::KeyValueStore> memoryStore = std::make_unique<scratchbaseext::FileKeyValueStore>(fullpath);
    std::unique_ptr<scratchbase::IDatabase> db(scratchbase::ScratchBase::createEmptyDB(dbname, memoryStore));
