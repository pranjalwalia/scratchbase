...
cxxopts::Options options("scratchbase-cli","CLI for ScratchBase");
...
int main(int argc, char* argv[]) {
    ...
    if (result.count("c") == 1) {
      ...
      std::unique_ptr<scratchbase::IDatabase> db(ScratchBase::createEmptyDB(dbname));
      return 0;
    }
    if (result.count("d") == 1) {
        if (result.count("n") == 0) {
        ...
        // destroy database
        std::string dbname(result["n"].as<std::string>());
        std::unique_ptr<scratchbase::IDatabase> db(ScratchBase::loadDB(dbname));
        db->destroy();
        return 0;
    }
    if (result.count("s") == 1) {
